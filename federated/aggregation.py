...
class SlimmableModelAccumulator(ModelAccumulator):
    ...
    def mask_add(self, model_idx, model, computable_body_layers, weight):
            ...
            for key in self._accum_state_dict:
                if len(self.local_state_dict) >0 and key in self.local_state_dict[model_idx]:
                    ...
                    if 'num_batches_tracked' in key:
                        ...
                        if key in computable_body_layers:
                            temp = weight * model.state_dict()[key]
                            self._accum_state_dict[key].data.add_(temp)
                            self._weight_sum[key] += weight
                            
        self._cnt += 1  # DO THIS at the END such that start from 0.
        
    def mask_hfl_add(self, model_idx, model, computable_body_layers, weight, max_slim_ratio=1.0, slim_bias_idx=0,
        ...
        with torch.no_grad():
            ...
            for slim_bias_idx in sorted(slim_bias_idxs):
                ...
                for key in self._accum_state_dict:
                    if key not in new_state_dict:
                        continue
                    if key not in computable_body_layers:
                        continue
    ...
    def mask_split_add(self, model_idx, model, computable_body_layers, weight, max_slim_ratio=1.0, slim_bias_idx=0,
            out_slim_bias_idx=None):
        """Only add params that are trained (defined by max_slim_ratio). Only weight
        params that are updated.
        Use weight = 1/n_accum to average.
        ...
        with torch.no_grad():
            ...
            for slim_bias_idx in sorted(slim_bias_idxs):
                ...
                for key in self._accum_state_dict:
                    if key not in new_state_dict:
                        continue
                    if key[1:] not in computable_body_layers[slim_bias_cntr]:
                        continue
                    new_tensor = new_state_dict[key]
                    if len(self.local_state_dict) > 0 and key in self.local_state_dict[model_idx]:
                        self.local_state_dict[model_idx][key].data.copy_(new_tensor)
