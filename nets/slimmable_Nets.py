...
class Block(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1):
        ...
        self.conv1 = SlimmableConv2d(in_planes, in_planes, kernel_size=3, stride=stride, padding=1, groups=1,
                                     bias=False)   
        self.bn1 = SlimmableBatchNorm2d(in_planes, track_running_stats=False)
        self.conv2 = SlimmableConv2d(in_planes, out_planes, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = SlimmableBatchNorm2d(out_planes, track_running_stats=False)
...
class MobileNetCifar(BaseModule, SlimmableMixin):
    ...
    def __init__(self, num_classes=10, track_running_stats=False, width_scale=1., bn_type='bn', slimmabe_ratios=None, dataset=None):
        ...
        self.conv1 = SlimmableConv2d(3, int(32 * width_scale), kernel_size=3, stride=1, padding=1,
                                     bias=False, non_slimmable_in=True)
        self.bn1 = SlimmableBatchNorm2d(int(32 * width_scale), track_running_stats=False)
        self.layers = self._make_layers(in_planes=int(32 *width_scale))
        self.linear = SlimmableLinear(int(width_scale*1024), num_classes,
